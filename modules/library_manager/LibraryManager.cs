...
public partial class LibraryManager : Node
{
	...
	private readonly string _appearanceFolder = Path.Combine( "Saves","Person","appearance" ); 
	private readonly string _appearanceVAPFolder = Path.Combine( "Custom","Atom","Person","Appearance" ); 
					...
					using( ZipArchive archive = ZipFile.OpenRead( file ) )
					{
						foreach( var entry in archive.Entries )
						{
							...
							if ( ( entry.FullName.StartsWith( _sceneFolder,StringComparison.OrdinalIgnoreCase ) &&
								 ...
								 ( entry.FullName.StartsWith( _appearanceFolder,StringComparison.OrdinalIgnoreCase ) &&
								   entry.FullName.EndsWith( ".json",StringComparison.OrdinalIgnoreCase ) ) )
							{
								AddScene( file,entry );
							...
							/*
							if( entry.FullName.StartsWith( _appearanceVAPFolder,StringComparison.OrdinalIgnoreCase ) &&
								 entry.FullName.EndsWith( ".vap",StringComparison.OrdinalIgnoreCase ) )
							{
								AddSceneVAP( file,entry );
							}
							*/
	...
	*/
	...
	{
		if( lookID >= 0 && lookID < _looks.Count )
			return _looks[lookID].PersonJSON;
		return string.Empty;
	}
	...
	public Dictionary Looks_GetTextures( int lookID )
	{
		if( lookID >= 0 && lookID < _looks.Count )
		{
			...
			using JsonDocument doc = JsonDocument.Parse( look.PersonJSON,new JsonDocumentOptions( ) { AllowTrailingCommas = true } );
			
			return GetTextures( doc.RootElement );
		}
					...
					foreach( JsonElement atom in atoms.EnumerateArray( ) )
					{
						if( atom.TryGetProperty( "type",out JsonElement typeElement ) && typeElement.ValueEquals( "Person" ) )
						{
							...
							foreach( JsonElement storable in storables.EnumerateArray( ) )
							{
								...
								{
									var texture = GetTextures( atom );

									bool add = false;
									if( texture.ContainsKey( "faceDiffuseUrl" ) &&
										( texture["faceDiffuseUrl"].AsString(  ).StartsWith( "SELF:/" ) ||
										( texture["faceDiffuseUrl"].AsString(  ).EndsWith(".jpg" ) ) ) ) 
									{
										add = true;
									}
									else
									{
										if( texture.ContainsKey( "torsoDiffuseUrl" ) &&
											( texture["torsoDiffuseUrl"].AsString(  ).StartsWith( "SELF:/" ) ||
											( texture["torsoDiffuseUrl"].AsString(  ).EndsWith(".jpg" ) ) ) ) 
										{
											add = true;
										}
									}
	...
	private void AddSceneVAP( string archive,ZipArchiveEntry entry )
	{
		try
		{
			using( StreamReader reader = new StreamReader( entry.Open( ) ) )
			{
				...
				{
					var root = doc.RootElement;
					JsonElement storables = root.GetProperty( "storables" );
					foreach( JsonElement storable in storables.EnumerateArray( ) )
					{
						if( storable.TryGetProperty( "id",out JsonElement idElement ) && idElement.ValueEquals( "geometry" ) )
						{
							...
							_looks.Add( new Looks
							{
								ID = _looks.Count,
								Archive = archive,
								PersonJSON = root.ToString( ), 
								Title = entry.Name.Replace( ".vap","" ),
								SceneFile = entry.FullName,
								PreviewImageFile = entry.FullName.Replace( ".json",".jpg" )
							} );
						}
	...
	public Dictionary GetTextures( JsonElement root )
	{
		//GD.Print( "JsonDocument" );
		if( root.TryGetProperty( "storables",out JsonElement storables ) )
		{
			//GD.Print( "storables" );
			foreach( JsonElement storable in storables.EnumerateArray( ) )
			{
