...
@export_range(-1,1) var pose_x : float = 0:
	set(value):
		pose_x = value
		update_pose(pose_x,pose_y,pose_z,height)
	get: return pose_x
...
func _init(skeleton: Skeleton3D,bone: int,limits : Dictionary = {}) -> void:
	_bone = bone
	_skeleton = skeleton
	_base_position = skeleton.get_bone_pose_position(bone)
