...
@export_range(0,1) var hips_vertical : float = 0:
	set(value):
		hips_vertical = value
		update_pose()
	get:
		return hips_vertical
@export_range(0,1) var frontal : float = 0:
	set(value):
		frontal = value
		update_pose()
	get:
		return frontal
@export_range(0,1) var straight : float = 0:
	set(value):
		straight = value
		update_pose()
	get:
		return straight
@export_range(0,1) var lateral : float = 0:
	set(value):
		lateral = value
		update_pose()
	get:
...
@export_range(0,1) var twist : float = 0:
	...
	get:
		return twist
@export_range(0,1) var ankle : float = 0:
	set(value):
		ankle = value
		update_pose()
	get:
...
func update_pose():
	...
	_hip.pose_z = hips_vertical
	
	_pelvis.pose_x = frontal
	_pelvis.pose_y = twist
	_pelvis.pose_z = lateral
	
	_knee.pose_x = straight
